...
const parseText = (text: string) => {
  // Replace **text** with <span class="text-primary font-bold">text</span>
  return text.replace(/\*\*(.*?)\*\*/g, '<span class="text-primary font-bold">$1</span>');
};
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 items-center group">
      ...
      <div className={`relative ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
        <div className={`${project.id === 'p3' ? 'aspect-[9/19] max-w-sm mx-auto' : 'aspect-[16/10]'} rounded-3xl overflow-hidden border border-primary/10 relative bg-surfaceHighlight/20 shadow-2xl group-hover:border-primary/20 transition-colors`}>
        ...
        <div className="absolute -bottom-5 left-6 right-6 flex flex-wrap gap-2 pointer-events-none z-30">
          {project.techStack.map((tech, i) => (
            <span key={i} className="px-3 py-1.5 bg-surface/90 backdrop-blur-md border border-primary/10 rounded-lg text-xs font-medium text-primary shadow-lg">
              {tech}
            </span>
          ))}
        </div>
      ...
      <div className={`${index % 2 === 1 ? 'lg:order-1' : ''}`}>
        ...
        {project.link && (
            <a 
              ...
              className="inline-flex items-center space-x-2 px-6 py-3 bg-primary/10 hover:bg-primary/20 text-primary font-medium rounded-full transition-all duration-300 border border-primary/5 hover:scale-105"
            >
              <Globe size={18} />
  ...
  return (
    <SectionWrapper className="bg-background">
      <div className="mb-12">
        ...
        {withEvolutionCard && (
          <div className="p-6 rounded-2xl bg-surfaceHighlight/30 border border-primary/5 max-w-4xl mt-8">
            <div className="flex items-start gap-4">
              <div className="p-3 bg-accent/10 rounded-xl text-accent">
                <Rocket className="w-6 h-6" />
              </div>
              <div>
                 <h4 className="text-primary font-bold mb-2 text-lg">我的 AI 全栈进化史 (2023 - 2025)</h4>
                 <p className="text-secondary text-sm leading-relaxed mb-4">
                   我将<span className="text-primary font-medium">技术探索视为最高级的娱乐</span>。
                   从 2023 年仅能利用 Prompt 做简单工具尝试，到 2025 年彻底爆发，利用<span className="text-primary font-medium">业余时间</span>打破了"Java 后端"的职能边界。
                   我不仅出于兴趣独立完成了<span className="text-primary font-medium">Web 全栈系统</span>和<span className="text-primary font-medium">iOS 原生应用</span>，
                   更将这种<span className="text-accent font-bold">业余时间积累的能力反哺公司</span>，在下方（汇报最后）展示的 MVP 中得到了终极验证。
                 </p>
                 <div className="flex flex-col md:flex-row gap-4 text-xs font-medium text-primary/80">
                    <div className="flex items-center gap-2 opacity-60">
                      <CheckCircle2 size={14} /> 2023: AI 助手 (Hobby)
                    </div>
                    <div className="hidden md:block text-primary/20">→</div>
